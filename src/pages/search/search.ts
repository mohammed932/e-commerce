...
export class SearchPage {
  ...
  getSearchData = function () {
    ...
    this.loading.show();
    this.config.Woocommerce.getAsync("products?per_page=100&search=" + this.search).then((data) => {
      this.loading.hide();
      this.searchResult = JSON.parse(data.body);
      this.showCategories = false;
      if (this.searchResult.length == 0) {
        this.translate.get("No Product found!").subscribe((res) => { this.alert.show(res); });
      }
