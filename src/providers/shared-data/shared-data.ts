...
export class SharedDataProvider {
  ...
  constructor(
    ...
    // //getting tab 1
    this.config.Woocommerce.getAsync("products").then((data) => {
      this.tab1 = JSON.parse(data.body);
      this.applicationRef.tick();
    });
    //getting tab 2
    this.config.Woocommerce.getAsync("products?on_sale=true").then((data) => {
      this.tab2 = JSON.parse(data.body);
      this.applicationRef.tick();
    });
    //getting tab 3
    this.config.Woocommerce.getAsync("products?page=1&order=desc&orderby=date&category=539").then((data) => {
      this.tab3 = JSON.parse(data.body);
      console.log('my this.tab3  :',this.tab3 );
  ...
  onePageCheckOut() {
    ...
    let onePage = 0;
    if (this.config.onePageCheckOut == true) onePage = 1;
